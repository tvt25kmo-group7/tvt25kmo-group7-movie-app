...
const TMDB_FETCH_GLOBAL_OPTIONS = {
    method: "GET",
    headers: {
...
async function searchMoviesAndTv(query, page = 1) {
    ...
    // Send the search request to the TMDB API
    const response = await fetch(url, TMDB_FETCH_GLOBAL_OPTIONS);
...
async function getNowPlayingMovies(page = 1) {
    ...
    // Construct the search parameters for the TMDB API request
    const searchParams = new URLSearchParams({
        region: "FI",
        language: "fi-FI",
        page: String(page),
    ...
    // Use the global fetch options for the TMDB API request
    const response = await fetch(url, TMDB_FETCH_GLOBAL_OPTIONS);
